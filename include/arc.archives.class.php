...
class Archives
{
        ...
        if(is_array($this->dtp->CTags))
        {
            foreach($this->dtp->CTags as $i=>$ctag)
            {
                ...
                else if($ctag->GetName()=='pagebreak')
                {
                    ...
                    {
                        $this->dtp->Assign($i,$this->GetPagebreak2($this->TotalPage,$this->NowPage,$this->ArcID));
                    }
                }
     ...
     * @return    string
     */
    function GetPagebreak2($totalPage, $nowPage, $aid)
    {
        if($totalPage==1)
        {
            return "";
        }
        $PageList = "<a class='pages_total'>共".$totalPage."页: </a>";
        $nPage = $nowPage-1;
        $lPage = $nowPage+1;
        if($nowPage==1)
        {
            $PageList.="<a class='pages_gray' href='#'>上一页</a>";
        }
        else
        {
            if($nPage==1)
            {
                $PageList.="<a class='pages_gray' href='".$this->NameFirst.".".$this->ShortName."'>上一页</a>";
            }
            else
            {
                $PageList.="<a class='pages_gray' href='".$this->NameFirst."_".$nPage.".".$this->ShortName."'>上一页</a>";
            }
        }
        for($i=1;$i<=$totalPage;$i++)
        {
            if($i==1)
            {
                if($nowPage!=1)
                {
                    $PageList.="<a class='pages_gray' href='".$this->NameFirst.".".$this->ShortName."'>1</a>";
                }
                else
                {
                    $PageList.="<a href='#' class='pages_gray current'>1</a>";
                }
            }
            else
            {
                ...
                if($nowPage!=$i)
                {
                    $PageList.="<a class='pages_gray' href='".$this->NameFirst."_".$i.".".$this->ShortName."'>".$n."</a>";
                }
                else
                {
                    $PageList.="<a class='pages_gray current' href='#'>{$n}</a>";
                }
            }
        }
        if($lPage <= $totalPage)
        {
            $PageList.="<a class='pages_gray' href='".$this->NameFirst."_".$lPage.".".$this->ShortName."'>下一页</a></li>";
        }
        else
        {
            $PageList.= "<a class='pages_gray nextpage' href='#'>下一页</a>";
        }
        return $PageList;
    }
