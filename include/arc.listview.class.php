...
class ListView
{
    ...
    function GetPageListST($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        if($totalpage<=1 && $this->TotalResult>0)
        {
            ...
            return "<a class='pages_total'>共 <strong>1</strong>页<strong>".$this->TotalResult."</strong>条记录</a>";
        }
        if($this->TotalResult == 0)
        {
            return "<a class=\"class='pages_total'\">共 <strong>0</strong>页<strong>".$this->TotalResult."</strong>条记录</a>";
        }
        $purl = $this->GetCurUrl();
        $maininfo = "<a class=\"pageinfo\">共 <strong>{$totalpage}</strong>页<strong>".$this->TotalResult."</strong>条</a>";
        ...
        if($this->PageNo != 1)
        {
            $prepage.="<a class='pages_gray' href='".str_replace("{page}",$prepagenum,$tnamerule)."'>上一页</a>";
            $indexpage="<a class='pages_gray' href='".str_replace("{page}",1,$tnamerule)."'>首页</a>";
        }
        else
        {
            $indexpage="<a class='pages_gray'>首页</a>";
        }

        //下一页,未页的链接
        if($this->PageNo!=$totalpage && $totalpage>1)
        {
            $nextpage.="<a class='pages_gray nextpage' href='".str_replace("{page}",$nextpagenum,$tnamerule)."'>下一页</a>";
            $endpage="<a class='pages_gray' href='".str_replace("{page}",$totalpage,$tnamerule)."'>末页</a>";
        }
        else
        {
            $endpage="<a class='pages_gray'>末页</a>";
        }
        ...
        for($j;$j<=$total_list;$j++)
        {
            if($j==$this->PageNo)
            {
                $listdd.= "<a class='pages_gray current'>$j</a>";
            }
            else
            {
                $listdd.="<a class='pages_gray' href='".str_replace("{page}",$j,$tnamerule)."'>".$j."</a>";
            }
        }
        $plist = '';
