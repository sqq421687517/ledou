...
if ( ! function_exists('GetParentTypename'))
{
    function GetParentTypename($id){
        global $dsql ;
        $reids = $dsql->GetOne("SELECT reid,typename FROM #@__arctype where id=$id");
        $reid = $reids['reid'];
        $currentTypename = $reids['typename'];
        ...
        $typename = $typename.','.$currentTypename;
        $typename = str_replace(['凡人修仙传','新闻中心','门派角色','玩家攻略','游戏视频','论坛'],['首页','资讯','门派','攻略','视频','论坛'],$typename);
...
if ( ! function_exists('getTypeConfig'))
{
    function getTypeConfig($typeid,$key)
    {
        global $dsql ;
        $topid = GetTopid($typeid);
        $row = $dsql->GetOne("SELECT content FROM #@__arctype where id=$topid");
        //$row['content'] = str_replace(["\n","\r\n","
//"],"",$row['content']);
        $content = array_filter(explode('<br />',$row['content']));
        foreach ($content as $k=>$v)
        {
            $content[$k] = trim($content[$k]);
            $contentList = explode('::',trim($content[$k]));
            if($contentList[0]==$key)
            {
