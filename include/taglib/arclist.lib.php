...
function lib_arclist( &$ctag, &$refObj )
{
    ...
    //传入typename参数时默认取当前点击栏目下typename栏目
    $typename = trim($ctag->GetAtt('typename'));
    if(!empty($typename))
    {
        global $dsql;
        $topid = GetTopid($envs['typeid']);
        if($typename=='当前栏目下所有文章')
        {
            $typeid = $topid;
        }
        else
        {
            $rowss = $dsql->GetOne("SELECT `id` FROM `#@__arctype` WHERE typename='$typename' AND topid='$topid' ");
            $typeid = $rowss['id'];
        }
    ...
    return lib_arclistDone
           (
             ...
             $tagid,$pagesize,$isweight,$ctag->GetAtt('today')
           );
}
...
function lib_arclistDone(&$refObj, &$ctag, $typeid=0, $row=10, $col=1, $titlelen=30, $infolen=160,
        ...
        $innertext='', $arcid=0, $idlist='', $channelid=0, $limit='', $att='', $order='desc', $subday=0, $noflag='',$tagid='', $pagesize=0, $isweight='N',$today='')
{
    ...
    if($idlist=='')
    {
        ...
        if($today>0)
        {
            $todayStart = gmmktime(0, 0, 0, gmdate('m'), gmdate('d'), gmdate('Y'));
            $orwheres[] = " arc.senddate > $todayStart ";
        }
        //关键字条件
        if($keyword!='')
        {
