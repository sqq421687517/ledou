...
file_put_contents('20171114_ledou.txt','111\r\n',FILE_APPEND);
require_once(DEDEDATA."/cache/inc_catalog_base.inc");
file_put_contents('20171114_ledou.txt','222\r\n',FILE_APPEND);
...
file_put_contents('20171114_ledou.txt','333\r\n',FILE_APPEND);
...
if($gotype=='' || $gotype=='mkallct')
{
    if($upnext==1 || $typeid==0)
    {
        if($typeid>0) 
        {
            file_put_contents('20171114_ledou.txt','444\r\n',FILE_APPEND);
...
else if($gotype=='mkall')
{
    ...
    $idArray = array();
    file_put_contents('20171114_ledou.txt','555\r\n',FILE_APPEND);
...
$reurl = '';
file_put_contents('20171114_ledou.txt','666\r\n',FILE_APPEND);
//更新数组所记录的栏目
if(!empty($tid))
{
    if(!isset($cfg_Cs[$tid]))
    ...
    {
        file_put_contents('20171114_ledou.txt','888\r\n',FILE_APPEND);
        require_once(DEDEINC."/arc.sglistview.class.php");
        file_put_contents('20171114_ledou.txt','sglistview\r\n',FILE_APPEND);
        $lv = new SgListView($tid);        
    }

    file_put_contents('20171114_ledou.txt','CountRecordstart\r\n',FILE_APPEND);
	// 这里统一统计
    $lv->CountRecord();
    ...
    file_put_contents('20171114_ledou.txt','CountRecordover\r\n',FILE_APPEND);
    //如果栏目的文档太多，分多批次更新
    if($ntotalpage <= $maxpagesize || $lv->TypeLink->TypeInfos['ispart']!=0 || $lv->TypeLink->TypeInfos['isdefault']==-1)
    {
        file_put_contents('20171114_ledou.txt','MakeHtmlmultistart\r\n',FILE_APPEND);
        $reurl = $lv->MakeHtml('', '', $isremote);
        file_put_contents('20171114_ledou.txt','MakeHtmlmultiover\r\n',FILE_APPEND);
        $finishType = TRUE;
    }
    else
    {
        file_put_contents('20171114_ledou.txt','MakeHtmlstart\r\n',FILE_APPEND);
        $reurl = $lv->MakeHtml($mkpage, $maxpagesize, $isremote);
        file_put_contents('20171114_ledou.txt','MakeHtmlover\r\n',FILE_APPEND);
        $finishType = FALSE;
...
if($nextpage >= $totalpage && $finishType)
{
    if($gotype=='')
    {
        ...
        /*$row = $dsql->GetOne("select typedir,sitepath,siteurl from `#@__arctype` where id=$tid");
        $reurl = $row['siteurl']. str_replace($row['sitepath'],'',$row['typedir']);*/
